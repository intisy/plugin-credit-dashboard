...
var OC_BASH = [
  ...
  '  exit 0',
  'fi',
  'if [ "$1" = "auth" ] && [ "$2" = "reset" ]; then',
  '  echo "Resetting OpenCode Antigravity proxy rate limits..."',
  '  bun -e "const fs=require(\'fs\');const path=require(\'path\');const h=require(\'os\').homedir();const ps=[path.join(h,\'.config\',\'opencode\',\'config\',\'antigravity-accounts.json\'),path.join(h,\'.config\',\'opencode\',\'antigravity-accounts.json\'),path.join(h,\'.config\',\'opencode\',\'data\',\'antigravity-accounts.json\')];let c=0;for(const p of ps){try{if(!fs.existsSync(p))continue;let d=JSON.parse(fs.readFileSync(p));for(let a of(d.accounts||[])){delete a.rateLimitRemaining;delete a.rateLimitReset;delete a.isRateLimited;delete a.coolingDownUntil;}fs.writeFileSync(p,JSON.stringify(d,null,2));c++;}catch(e){}}console.log(c?\'Rate limits cleared across \'+c+\' file(s).\':\'No accounts files found.\');"',
  '  exit 0',
  'fi',
...
async function installOcLauncher() {
  ...
  var tuiSrc = join(import.meta.dirname, "..", "oc-tui.js");
